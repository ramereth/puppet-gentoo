...
type_class = Puppet::Type.type(:package_mask)
...
describe type_class do
  ...
  describe "when validating the target property" do
    it "should default to the provider's default target" do
      @class.new(:name => "sys-devel/gcc").should(:target).should == "/etc/portage/package.mask/defaulttarget"
    end
    it "should munge targets that do not specify a default path" do
      @class.new(:name => "sys-devel/gcc", :target => "gcc").should(:target).should == "/etc/portage/package.mask/gcc"
    end
  end
end
